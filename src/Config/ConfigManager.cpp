...
bool ConfigManager::load(const std::string& path) {
    ...
    if (!file.is_open()) {
        // 文件不存在，创建默认配置
        cm::logger.info("Config file not found, creating a new one at: {}", path);
        return save();
    }
    ...
    try {
        ...
        *mConfig = user_json.get<Config>();
        ...
        cm::logger.error("Failed to parse config file: {}. Please check for syntax errors.", e.what());
        return false;
    }
    ...
    if (!missing_keys.empty()) {
        cm::logger.warn("Your config file is missing some keys. The plugin will use default values for them.");
        cm::logger.warn("This won't affect functionality, but you may want to add them to your config.json:");
        for (const auto& key : missing_keys) {
            cm::logger.warn("- {}", key);
        }
        cm::logger.warn("You can also delete your config.json to generate a new one with all keys on next startup.");
    }
...
bool ConfigManager::save() {
    try {
        ...
        cm::logger.error("Failed to save config file: {}", e.what());
        return false;
    }
    return true;
}
